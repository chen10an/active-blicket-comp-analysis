...
parser <- add_option(parser, c("-b", "--prior_binsize"), type="character", default="point3FformBinSize", help="Name of the scala trait corresponding to the bin size to use for the joint prior over bias and gain. [default %default]")
...
parser <- add_option(parser, "--width", type="double", default=NA)
...
# SET THESE VARS -----
SAVEDIR <- file.path(args$cache_dir, "2022-04-18_noP1", sprintf("%.2f-%.2f_%.2f-%.2f_%.2f", args$bias_shape, args$bias_scale, args$gain_shape, args$gain_scale, args$width)) # main model starting from phase 2
createDirs(SAVEDIR)

# sigmoid grid:
prior_filename <- sprintf("bias-shape=%.2f-scale=%.2f_gain-shape=%.2f-scale=%.2f_width=%.2f_grid.csv", args$bias_shape, args$bias_scale, args$gain_shape, args$gain_scale, args$width)
print(sprintf("Using the prior file %s with bin size %s. Adding this file to the model's save directory:", prior_filename, args$prior_binsize))
command <- sprintf("rsync --archive --update --compress --progress %s %s", file.path(args$cache_dir, "gamma_priors", prior_filename), SAVEDIR)
print(sprintf("%s", command))
system(command)

grid <- fread(file.path(args$cache_dir, "gamma_priors", prior_filename))
s + '
...
getModelInitStr <- function(prior_str) {
  ...
  # main model
  sprintf('new PhaseLearner(%s) with %s', prior_str, args$prior_binsize)
}
s + '
